...
namespace EDDS.Information.Helpfolder
{
    public partial class Signal : UserControl
    {
        ...
        private void UpdateRows()
        {
            ...
            DataTable DT = DB.ExecuteReader("SELECT id, name FROM edds_keywords ORDER BY id");
            foreach (DataRow row in DT.Rows)
            {
                dataGridView1.Rows[dataGridView1.Rows.Add(row["id"], row["name"], row["desc"])].ReadOnly = true;
            }
